...
**Эффективность:** в **десятки раз выше меди** по теплопроводности (в типичных условиях ~4–130×, в идеальных до ~250×)!
...
- Вода (+50°C до +150°C): только для замкнутых внутренних контуров (пилотируемые модули), **не** для наружных тепловых труб — при входе в тень Земли замёрзнет и разорвёт капиллярную структуру

**Пределы мощностей (Q_max):** тепловая труба — не «сверхпроводник», а динамическое устройство с жёстким потолком тепловой мощности. Если сервер резко сбросит пиковую нагрузку, труба может «запереться»:

- **Звуковой предел** — скорость пара достигает скорости звука и блокирует поток.
- **Предел уноса** — быстрый пар срывает капли жидкости с фитиля и гонит их обратно, осушая зону испарения.

В обоих случаях передача тепла прекращается, и чип может сгореть за секунды. Поэтому для каждой трубы заранее считают максимальную мощность Q_max и закладывают запас — пиковые нагрузки должны быть в пределах паспортного режима.
...
| **OSR**                 | 0.92-0.94        | 0.05-0.13      | 0.07 | Прецизионные радиаторы       |
...
```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌──────────┐
│ Сервер (чип)│ ─► │ Контакт     │ ─► │Тепловые     │ ─► │ Жидкостный  │ ─► │ Радиатор │ ─► Космос
│   +70°C     │    │ чип→корпус  │    │ трубы       │    │ контур      │    │  +50°C   │    ИК-излучение
...
При Q = 500 Вт суммарный перепад 20°C раскладывается на все звенья цепи: чип → корпус сервера → трубы → жидкость → радиатор. На каждом из четырёх стыков теряется по ~5°C, и даже при этом +70°C на чипе превращаются в +50°C на поверхности радиатора. Контакты металлов всегда дают термическое сопротивление — именно поэтому сервер заведомо горячее радиатора.
...
- **Поворотные радиаторы** — панели на приводах непрерывно вращаются, оставаясь ребром к Солнцу. На МКС их держат кольцевые приводы **SARJ** (Solar Alpha Rotary Joint), для охлаждающих панелей — аналогичные механизмы.
...
| **Кубсат (3U)**      | 10-50 Вт       | Пассивные радиаторы на корпусе | 0.05-0.1 м² (вся поверхность ~0.16 м²) |
...
2. **МКС** гоняет по радиаторам аммиак. При нормальном давлении он замерзает при **-78°C**, но в системе давление выше, а у аммиака кривая плавления имеет положительный наклон — значит точка замерзания **поднимается** (до примерно -74°C). Это серьёзная проблема для инженеров, а не подсказка.
